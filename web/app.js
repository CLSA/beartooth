...
cenozo.controller("HeaderCtrl", [
  ...
  async function (
    ...
    CnSession.alertHeader =
      null == CnSession.user.assignment
        ...
        : "You are currently in a " +
          CnSession.user.assignment.type +
          " assignment";
    CnSession.onAlertHeader = async function () {
      ...
      if (
        angular.isDefined(cenozoApp.module("assignment").actions[controlName])
      ) {
        ...
        await CnModalMessageFactory.instance({
          title:
            "Switch Roles For " +
            CnSession.user.assignment.type.ucWords() +
            " Assignment",
          message:
    ...
    var logoutOperation = $scope.operationList.findByProperty(
      "title",
      "Logout"
    );
    ...
    logoutOperation.execute = async function () {
      ...
      async function showAssignmentExists(assignmentType) {
        ...
        var hasAccess = angular.isDefined(
          cenozoApp.module("assignment").actions[controlName]
        );

        await CnModalMessageFactory.instance({
          title:
            "Active " +
            (assignmentType ? assignmentType.ucWords() + " " : "") +
            "Assignment Detected",
          message:
            "You cannot log out while in an open assignment!\n\n" +
            (hasAccess
