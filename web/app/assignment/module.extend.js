cenozoApp.extendModule({
  ...
  create: (module) => {
    ...
    function getAssignmentControlDirective(
    ...
    cenozo.providers.directive("cnAssignmentHomeControl", [
      ...
      function (CnAssignmentControlFactory, CnSession, $window) {
        return getAssignmentControlDirective(
          CnAssignmentControlFactory,
          CnSession,
          $window
        );
        ...
        function extendObject(object) {
          angular.extend(object, {
            ...
            transitionToParentViewState: async function (subject, identifier) {
              await $state.go(subject + ".view", {
                type: object.viewModel.record.type,
    ...
    cenozo.providers.factory("CnAssignmentControlFactory", [
      ...
      function (
        ...
        var object = function (root) {
          ...
          this.type = null;
          if ("assignment.home_control" == $state.current.name)
            this.type = "home";
          else if ("assignment.site_control" == $state.current.name)
            this.type = "site";
          if (null == this.type) {
            if ("home" != $state.params.type && "site" != $state.params.type) {
              $state.go("error.404");
              throw (
                'Cannot find user matching assignment type "' +
                $state.params.type +
                '", redirecting to 404.'
              );
            }
          ...
          angular.extend(this, {
            ...
            onLoad: async function (closeScript) {
              ...
              try {
                  ...
                  onError: async function (error) {
                    ...
                    if (307 == error.status) {
                      ...
                      CnSession.alertHeader =
                        "You are currently in a " + self.type + " assignment";
                      CnSession.setBreadcrumbTrail([
                        { title: "Assignment" },
                        { title: "Wrong Site" },
                      ]);
                ...
                CnSession.alertHeader =
                  "You are currently in a " + this.type + " assignment";
                ...
                if (this.assignment.type != this.type) {
                  ...
                  try {
                    ...
                    this.activePhoneCall =
                      0 < len &&
                      null === this.phoneCallList[len - 1].end_datetime
                  ...
                  try {
                    ...
                    console.error(
                      "Failed to get previous assignment information"
                    );
                  } finally {
                    this.isPrevAssignmentLoading = false;
                  }
          ...
          async function init(object) {
            ...
            if ("home" == object.type) {
              ...
              object.participantModel.addColumn("address_summary", {
                title: "Address",
              });
            } else {
              // 'site' == object.type
