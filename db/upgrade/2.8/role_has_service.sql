...
CREATE PROCEDURE patch_role_has_service()
  BEGIN
    ...
    SET @sql = CONCAT(
      ...
      "AND service.subject IN( 'alternate_type', 'equipment', 'equipment_loan', 'equipment_type', 'notation' ) ",
    ...
    SET @sql = CONCAT(
      ...
      "SELECT role.id, service.id ",
      "FROM ", @cenozo, ".role, service ",
      "WHERE role.name IN( 'coordinator', 'curator', 'interviewer', 'interviewer+' ) ",
      "AND service.subject IN( 'equipment', 'equipment_type', 'identifier', 'participant_identifier' ) ",
      "AND service.method = 'GET' ",
      "AND service.restricted = 1"
    );
    PREPARE statement FROM @sql;
    ...
    SET @sql = CONCAT(
      ...
      "SELECT role.id, service.id ",
      "FROM ", @cenozo, ".role, service ",
      "WHERE role.name IN( 'coordinator', 'curator' ) ",
    ...
    SET @sql = CONCAT(
      ...
      "AND service.subject = 'equipment_loan' ",
      "AND service.restricted = 1"
    );
    PREPARE statement FROM @sql;
    EXECUTE statement;
