...
class queue extends \cenozo\database\record
{
  ...
  protected static function prepare_queue_query( $queue, $select, $modifier )
  {
    ...
    {
      // link to appointment table and make sure the appointment hasn't been resolved
      // (by design, there can only ever be one unresolved appointment per interview)
    ...
    // Make sure there is no unresolved appointment.  By design there can only be one unresolved appointment
...
IFNULL( study_consent.accept, current_qnaire.allow_missing_consent ) AS study_consent_accept,
...
LEFT JOIN participant_first_address ON participant.id = participant_first_address.participant_id
LEFT JOIN address AS first_address ON participant_first_address.address_id = first_address.id
